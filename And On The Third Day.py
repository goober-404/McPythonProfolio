...
enemyEasterEgg = pygame.image.load('furious-man-with-clenched-fists-and-bared-teeth-exudes-intense-anger-reflecting-frustration-and-tension-with-a-dangerous-presence-free-photo.webp')
...
pause_active = False
pause_selected = 0
pause_menu_items = ['up', 'down', 'left', 'right', 'save', 'resume']
rebind_mode = False
rebind_action = None

main_menu_active = True
main_menu_selected = 0
main_menu_items = ['New Game', 'Load Game', 'Quit']
...
enemyEasterEgg = enemyEasterEgg.convert_alpha()
...
def load_game(slot):
    try:
        with open(os.path.join(save_dir, f'save_{slot}.pkl'), 'rb') as f:
            state = pickle.load(f)
        global playerSpriteSFrect, enemies, kill_count, holylightBullets, boost, boostVector, knockbackVector, acceptingNewVector, inRange, isWalking
...
def draw_main_menu():
    ...
    for i, item in enumerate(main_menu_items):
        ...
        screen.blit(text, (w//2 - text.get_width()//2, 300 + i*50))
    pygame.display.flip()

def draw_easter_egg():
    screen.fill(BLACK)
    angle = (pygame.time.get_ticks() // 5) % 360  # faster rotation
    scaled = pygame.transform.scale(enemyEasterEgg, (420, 420))
    ...
    screen.blit(top_text, (w//2 - top_text.get_width()//2, 80))
    screen.blit(bottom_text, (w//2 - bottom_text.get_width()//2, h - 120))
    pygame.display.flip()
            ...
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_UP:
                    ...
                    if main_menu_items[main_menu_selected] == 'New Game':
                        ...
                        # else no saves
                    elif main_menu_items[main_menu_selected] == 'Quit':
                        gameLoop = False
        clock.tick(FPS)
        continue
